...
async function takeVerificationScreenshots() {
  ...
  try {
    ...
    const productCard = desktopPage.locator('.product-card, [class*="product"], a[href*="/product"]').first();
    if (await productCard.count() > 0) {
      await productCard.click();
      await desktopPage.waitForTimeout(1500);
    }

    // Ищем контейнер с размерами
    const sizesContainer = desktopPage.locator('[class*="size"], .sizes, .product-sizes, .size-selector').first();
    ...
    // Hover на кнопку размера
    const sizeButton = desktopPage.locator('.size-option, [class*="size-btn"], button:has-text("M"), button:has-text("L"), [class*="size"] button').first();
